 ...
 *
 * @param element - The target HTMLElement. If null, the function does nothing.
 * @returns void
 */
export function scrollToElement(element: HTMLElement | null) {
  if (element) {
    const offset = 60; // 60px of space above the element
...
export function getTeamNameArray(
  i: number,
  gameFormat: GameFormat,
  bidWinners: number[],
  nonBidWinners: number[],
  teams: Team[]
): string[] {
  ...
  let teamNames = [];
  if (bidWinners.includes(i)) {
    bidWinners.forEach((k) => {
      teamNames.push(teams[k].name);
    });
  } else {
    nonBidWinners.forEach((k) => {
      teamNames.push(teams[k].name);
    });
  }
  return teamNames;
